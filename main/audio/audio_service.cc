...
void AudioService::Start() {
    if (!service_stopped_) {
        ESP_LOGW(TAG, "Start ignored: already running");
        return;
    }
    if (codec_ == nullptr) {
        ESP_LOGE(TAG, "Start failed: codec not initialized");
        return;
    }
    ...
    // 栈放 SPIRAM：内部 DRAM 碎片时 xTaskCreate 易失败；TCB 仍在内部
    constexpr UBaseType_t kStackCaps = MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT;

#if CONFIG_USE_AUDIO_PROCESSOR
    /* Start the audio input task */
    BaseType_t ok_in = xTaskCreatePinnedToCoreWithCaps(
        [](void* arg) {
            AudioService* audio_service = (AudioService*)arg;
            audio_service->AudioInputTask();
            vTaskDeleteWithCaps(NULL);
        },
        "audio_input", 2048 * 3, this, 8, &audio_input_task_handle_, 0, kStackCaps);
    if (ok_in != pdPASS) {
        ESP_LOGE(TAG,
                 "Failed to create audio_input (SPIRAM free=%u largest=%u)",
    ...
    BaseType_t ok_out = xTaskCreateWithCaps(
        [](void* arg) {
            AudioService* audio_service = (AudioService*)arg;
            audio_service->AudioOutputTask();
            vTaskDeleteWithCaps(NULL);
        },
        "audio_output", 2048 * 2, this, 4, &audio_output_task_handle_, kStackCaps);
    if (ok_out != pdPASS) {
        ESP_LOGE(TAG, "Failed to create audio_output");
...
#else
    /* Start the audio input task */
    BaseType_t ok_in = xTaskCreateWithCaps(
        [](void* arg) {
            AudioService* audio_service = (AudioService*)arg;
            audio_service->AudioInputTask();
            vTaskDeleteWithCaps(NULL);
        },
        "audio_input", 2048 * 2, this, 8, &audio_input_task_handle_, kStackCaps);
    if (ok_in != pdPASS) {
        ESP_LOGE(TAG, "Failed to create audio_input");
        audio_input_task_handle_ = nullptr;
        service_stopped_ = true;
        return;
    }
    ...
    BaseType_t ok_out = xTaskCreateWithCaps(
        ...
        "audio_output", 2048, this, 4, &audio_output_task_handle_, kStackCaps);
    if (ok_out != pdPASS) {
        ESP_LOGE(TAG, "Failed to create audio_output");
        audio_output_task_handle_ = nullptr;
        service_stopped_ = true;
    ...
    /* opus_codec: stack in PSRAM (large), TCB in INTERNAL — mirrors wake_word encode */
    if (opus_codec_stack_ == nullptr) {
        opus_codec_stack_ = static_cast<StackType_t*>(heap_caps_malloc(
            kOpusCodecStackWords * sizeof(StackType_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    }
    if (opus_codec_tcb_ == nullptr) {
        opus_codec_tcb_ = static_cast<StaticTask_t*>(
            heap_caps_malloc(sizeof(StaticTask_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
    }
    if (opus_codec_stack_ == nullptr || opus_codec_tcb_ == nullptr) {
        ESP_LOGE(TAG, "Failed to alloc opus_codec stack/tcb (stack=%p tcb=%p)",
    ...
    opus_codec_task_handle_ = xTaskCreateStatic(
        ...
        "opus_codec", kOpusCodecStackWords, this, 2, opus_codec_stack_, opus_codec_tcb_);
    if (opus_codec_task_handle_ == nullptr) {
        ESP_LOGE(TAG, "Failed to create opus_codec static task");
        service_stopped_ = true;
        xEventGroupSetBits(event_group_, AS_EVENT_AUDIO_TESTING_RUNNING | AS_EVENT_WAKE_WORD_RUNNING |
                           AS_EVENT_AUDIO_PROCESSOR_RUNNING);
        return;
    }
    ESP_LOGI(TAG, "Audio tasks started (input/output stack SPIRAM, opus_codec stack %u words SPIRAM)",
             static_cast<unsigned>(kOpusCodecStackWords));
}
...
bool AudioService::ReadAudioData(std::vector<int16_t>& data, int sample_rate, int samples) {
    if (codec_ == nullptr) {
        ESP_LOGE(TAG, "ReadAudioData: codec not initialized");
        return false;
    }
...
void AudioService::SetDecodeSampleRate(int sample_rate, int frame_duration) {
    // 必须持 audio_queue_mutex_：与 ResetDecoder / 其它任务并发时，
    // 裸 reset() 会让 opus_decoder_ 短暂为空，ResetState 锁已销毁 mutex → abort。
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
...
void AudioService::SetModelsList(srmodel_list_t* models_list) {
    // 仅供 AFE/语音处理；本产品不建唤醒词对象，避免 audio_detection 等常驻任务
