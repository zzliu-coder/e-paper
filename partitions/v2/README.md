...
### 16MB + fontpack (`16m_fontpack.csv`)
- Same OTA layout as `16m.csv`
- `font_data` (raw, subtype `0x40`): **6MB** at `0x800000` — for `fonts.fontpack` + `esp_partition_mmap`
- `assets` (spiffs): **2MB** (shrunk to make room)

### 32MB + fontpack (`32m_fontpack.csv`)
- `font_data`: **6MB** at `0xA00000`
- `assets`: **10MB**

Flash the pack after changing partition table:
...
parttool.py --port PORT write_partition --partition-name font_data \
  --input tools/fontpack/fonts.fontpack
```

Device API: `font_loader_init_flash()` + `get_glyph()` in `tools/fontpack/esp32/`.
