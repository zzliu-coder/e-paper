...
void WifiConfigurationAp::Start()
{
    ...
    esp_timer_create_args_t timer_args = {
        .callback = [](void* arg) {
            ...
            if (!self->is_connecting_ && !self->lp_paused_) {
                esp_wifi_scan_start(nullptr, false);
            }
        },
        .arg = this,
...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    // APSTA 必须带 STA netif，否则 esp_wifi_stop 异步停网时可能对空指针做 esp_netif_stop
...
void WifiConfigurationAp::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        // 租约持有中：不抢 AP 表，仍排队下次周期扫，避免产测扫网后配网列表停更
...
void WifiConfigurationAp::PauseForLp()
{
    if (lp_paused_) {
        return;
    }
    // 与 STA PauseForLp 一致：先置位，避免 stop 过程中扫描定时器/事件再开扫
    lp_paused_ = true;
    ...
    ESP_LOGI(TAG, "config AP paused for LP (scan/httpd/dns/radio off)");
}

esp_err_t WifiConfigurationAp::ResumeFromLp()
{
    if (!lp_paused_) {
        return ESP_OK;
    }
    ...
    // 须先清 pause：esp_wifi_start 可能同步抛事件
    lp_paused_ = false;
    ...
    ESP_LOGI(TAG, "config AP resumed from LP");
    return ESP_OK;
}
